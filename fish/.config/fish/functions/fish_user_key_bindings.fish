...
function __git_fzf_git_status
    ...
    git status --short | fzf -m --preview 'git diff HEAD -- {-1}' | cut -c4- | sed 's/.* -> //' | while read -l r
        set result $result $r
    end
...
function __git_fzf_git_branch
    __git_fzf_is_in_git_repo; or return
    git branch -a | fzf -m --preview 'git hist (echo {} | sed s/^..// | cut -d" " -f1)' | sed 's/^..//' | cut -d' ' -f1 | sed 's#^remotes/##' | while read -l r
        set result $result $r
    end
...
function __git_fzf_git_tag
    __git_fzf_is_in_git_repo; or return
    git tag --list | fzf -m --preview 'git show {}' | while read -l r
        set result $result $r
    end
...
function __git_fzf_git_log
    __git_fzf_is_in_git_repo; or return
    git hist | fzf -m --reverse --preview 'git show (echo {} | grep -o "[a-f0-9]\{7,\}")' | sed -E 's/.*([a-f0-9]{7,}).*/\1/' | while read -l r
        set result $result $r
    end
